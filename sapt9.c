...
void verificarePropozitii(char* continut, char caracter, int PF)
{
    ...
    else
    {
        ...
        if (WIFEXITED(status) && WEXITSTATUS(status) == 0)
        {
            ...
            int nrBytesCititi = read(PF, bufferRezultat, sizeof(bufferRezultat));
        
            if (nrBytesCititi == -1)
            {
                perror("eroare la citirea rezultatului din script!\n");
                exit(10);
            }
...
int main(int argc, char* argv[])
{
    ...
    while ((dirInp = readdir(dirI)) != NULL) 
    {
            ...
            else
            {
                if(S_ISREG(var.st_mode))
                {
                    ...
                    else
                    {
                        ...
                        int generare_continut_fisier = fork();  //se creeaza un nou proces pentru fisierele care nu sunt bmp

                        if (generare_continut_fisier == -1)
                        {
                            perror("eroare la fork pentru generarea continutului");
                            exit(9);
                        }

                        if (generare_continut_fisier == 0)
                        {
                            close(PF[0]); // inchide capatul de citire al pipe-ului

                            verificarePropozitii(continut_citit, *argv[3], PF[1]);

                            close(PF[1]);  //inchidem capatul de scriere al pipe-ului
                            exit(11);
                        }
                        ...
                        else
                        {
                            ...
                            waitpid(generare_continut_fisier, NULL, 0);

                            char rezultat[BUFF_SIZE];
                            int bytesRezultat = read(PF[0], rezultat, sizeof(rezultat));
                            ...
                            printf("am identificat %d propozitii corecte care contin caracterul %c\n", numar_prop, *argv[3]);        
        ...
        else
        {
            int status;
            waitpid(pid, &status, 0);
            printf("S-a incheiat procesul pentru %s cu PID-ul %d si statusul %d\n", dirInp->d_name, pid, status);
        }
